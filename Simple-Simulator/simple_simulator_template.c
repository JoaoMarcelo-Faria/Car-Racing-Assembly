...
#define DIVR 38     // "100110"; -- DIVR Rx Ry Rz           -- Rx <- (Ry + Rz/2) / Rz   Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case STORE:
					// STORE END Rx -> Busca endereço na memória (PC) onde guardar Rx
					selM1 = sPC;
					RW = 0;
					LoadMAR = 1;       // Guarda endereço de destino no MAR
					IncPC = 1;
					state=STATE_EXECUTE;
					break;
				...
				case LXOR:
				case LNOT:
        			selM3 = ry;        // Usa ry como primeiro operando
        			selM4 = rz;        // Usa rz como segundo operando
        			OP = opcode; 
        			selM2 = sULA; 
        			LoadReg[rx] = 1; 
        			selM6 = sULA; 
				...
				case INC:
        			selM3 = rx; 
        			selM4 = 8; // M4=1
        			if (pega_pedaco(IR, 6, 6) == 0) OP = ADD; else OP = SUB;
        			selM2 = sULA; 
        			LoadReg[rx] = 1; 
        			selM6 = sULA; 
        			LoadFR = 1;
				...
				case CALL:
					// Verifica condição (Cópia da lógica do JMP)
					COND = pega_pedaco(IR,9,6);
					if((COND == 0)
							|| (FR[0]==1 && (COND==7)) || ((FR[2]==1 || FR[0]==1) && (COND==9))
							|| (FR[1]==1 && (COND==8)) || ((FR[2]==1 || FR[1]==1) && (COND==10))
							|| (FR[2]==1 && (COND==1)) || (FR[2]==0 && (COND==2))
							|| (FR[3]==1 && (COND==3)) || (FR[3]==0 && (COND==4))
							|| (FR[4]==1 && (COND==5)) || (FR[4]==0 && (COND==6))
							|| (FR[5]==1 && (COND==11))|| (FR[5]==0 && (COND==12))
							|| (FR[6]==1 && (COND==14))|| (FR[9]==1 && (COND==13)))
					{
						// Se condição verdadeira: Push PC (Endereço do argumento) para a pilha
						selM1 = sSP;
						selM5 = sPC;
						...
						// Se falso, salta o argumento (imediato)
						IncPC = 1;
						state = STATE_FETCH;
					}
					break;
				...
				case PUSH:
					...
					// Verifica bit 6 para saber se é Rx ou FR
					if (pega_pedaco(IR, 6, 6) == 0) { // PUSH Rx
						selM3 = rx;
						selM5 = sM3;
					} else { // PUSH FR
						selM3 = 8; // 8 seleciona FR na lógica do Mux3 (ver main)
						selM5 = sM3;
					}
					state = STATE_FETCH;
					break;
				...
				case POP:
					// Prepara para ler da pilha: SP++
					IncSP = 1;
					state = STATE_EXECUTE;
					break;

				case RTS:
					// Prepara para ler retorno da pilha: SP++
					IncSP = 1;
					state = STATE_EXECUTE;
					break;
		...
		case STATE_EXECUTE:
			...
			switch(opcode){
				...
				case CALL:
					// Finaliza o CALL: PC <- Mem[PC] (O endereço de salto imediato)
					selM1 = sPC;
					RW = 0;
					LoadPC = 1;
					state = STATE_FETCH;
				...
				case POP:
					// Finaliza o POP: Rx/FR <- M[SP]
					selM1 = sSP;
					RW = 0;
					if (pega_pedaco(IR, 6, 6) == 0) { // POP Rx
						selM2 = sDATA_OUT;
						LoadReg[rx] = 1;
					} else { // POP FR
						selM6 = sDATA_OUT;
				...
				case RTS:
					// Passo 1 do RTS: PC <- M[SP] (Restaura endereço do argumento do CALL)
					selM1 = sSP;
					RW = 0;
					LoadPC = 1;
					state = STATE_EXECUTE2; // Precisa de mais um ciclo para saltar o argumento
					break;
		...
		case STATE_EXECUTE2:
			// Passo 2 do RTS: PC++ (Salta o argumento do CALL original)
			if (opcode == RTS) {
				IncPC = 1;
			}
			state = STATE_FETCH;
			...
			switch(OP) {
				...
				case DIVR: 
					if(y==0) {
						...
						// Fórmula: (Dividendo + (Divisor / 2)) / Divisor
						// Como estamos em C com inteiros, y/2 já trunca corretamente para baixo
						result = (x + (y / 2)) / y;
						auxFRbits[DIV_BY_ZERO] = 0;
					}
					break;
